...
4) quit'''
    ...
    def send_packet(self, user_input):
        ...
        while not self.ack_received: # wait for the ack from the server
            if time.time() - start_time >= util.TIME_OUT:
                j = 0
                # Timeout occurred
                logger.debug("***Timeout occurred while waiting for start ack.***")
        ...
        for chunk in chunks:
          ...
          #  logger.debug("client sending packet:")
            ...
            while not self.ack_received: # wait for the ack from the server
                # if time.time() - start_time >= util.TIME_OUT:
                #     # if num_resends > 10:
                #     #     logger.debug("Gave up resending")
                #     #     break
                #     j = 0
                #     # Timeout occurred
                #     logger.debug("***Timeout occurred while waiting for data ack.***, username: %s", self.name)
                #     self.sock.sendto(packet, (self.server_addr, self.server_port)) # resend the packet
                #     num_resends += 1
                pass
            self.ack_received = False
    ...
    def receive_handler(self):
        ...
        '''
        server_message = "" # Build the message from the chunks the server is sending
        while True:
            ...
            if msg_type == "ack":
                #logger.debug("received ack %d", int(seqno))
                self.ack_received = True
                continue
            elif msg_type == "start":
                logger.debug("received start %d", int(seqno))
                packet = util.make_packet(msg_type="ack", seqno=int(seqno)+1).encode() # ack client's start packet
                self.sock.sendto(packet, address) # send the ack
                server_message = "" # reset server message so as to not include data from last connection
                continue
            elif msg_type == "data":
                logger.debug("received data %d", int(seqno))
                parsed_pkt = util.parse_packet(packet.decode())
                server_message += parsed_pkt[2]
           #     logger.debug("client msg chunk: %s", parsed_pkt)
                ...
                packet = util.make_packet(msg_type="ack", seqno=int(seqno)+1).encode() # ack client's start packet
                self.sock.sendto(packet, address) # send the ack
              #  logger.debug("complete message from server: %s", server_message)
            msg_type_arr = server_message.split(' ')
          #  logger.debug(data)
          #  logger.debug(msg_type_arr)
            ...
            #     self.sock.shutdown(socket.SHUT_WR)
                ...
                print("msg:",sender+":",server_message[i:])
